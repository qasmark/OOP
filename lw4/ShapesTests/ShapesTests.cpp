...
TEST_CASE("Testing CLineSegment")
{
...
TEST_CASE("Testing ShapesHandler")
{
	WHEN("Creating Line using ShapesHandler")
	{
		THEN("Line has been successfully created")
		{
			std::istringstream inputStream("line 0 0 5 5 ff00ffff \nprintAllShapes");
			std::ostringstream outputStream;
			const ShapesHandler shapesHandler(inputStream, outputStream);
	...
	WHEN("Creating Triangle using ShapesHandler")
	{
	...
	WHEN("Creating Rectangle using ShapesHandler")
	{
	...
	WHEN("Creating Circle using ShapesHandler")
	{
...
TEST_CASE("Program has ability to draw figures")
{
	WHEN("Program draws line")
	{
		THEN("Line appears on window")
		{
			...
			const HexColor outlineColor = 0xfff;
			const CLineSegment line(start, end, outlineColor);
